...
if ( !defined( 'FRAKTJAKT_API_VERSION' ) ) {
    define( 'FRAKTJAKT_API_VERSION', '3.10.0' );
}
if ( !defined( 'FRAKTJAKT_PLUGIN_VERSION' ) ) {
    define( 'FRAKTJAKT_PLUGIN_VERSION', '2.3.3' );
}
...
function Fraktjakt_order_meta_box_content()
{
    ...
    if (!empty($fraktjakt_tracking_link)) {
	        // Fraktjakt button to order
	        echo '&nbsp; <input id="fraktjakt_trace_button" class="button-primary" type="button" value="'.__( 'Trace shipment', 'fraktjakt-shipping-for-woocommerce' ).'" title="'.__( 'Trace the shipment', 'fraktjakt-shipping-for-woocommerce' ).'">';    
	        echo "<script type=\"text/javascript\" >
...
/** ---------------------------------------------------
 *   Add tracking link and agent selection to emails
 *  ---------------------------------------------------
 */
...
function custom_woocommerce_email_order_meta_fields( $fields, $sent_to_admin, $order ) {
    $tracking_link=get_post_meta( $order->get_id(), 'fraktjakt_tracking_link', true);  
		$agent_selection_link==get_post_meta( $order->get_id(), 'agent_selection_link', true);
		$options=get_option('woocommerce_fraktjakt_shipping_method_settings');
    if (($options['enable_tracking_in_email'] == 'yes') && (!empty($tracking_link))){
	    $fields['track_shipment'] = array(
	        'label' => __('Follow the shipment', 'fraktjakt-shipping-for-woocommerce'),
	        'value' => "<a href='".$tracking_link."' class='tracking_button button button-primary'>".__('Track your package', 'fraktjakt-shipping-for-woocommerce')."</a>"
	        ); 
    };
    return $fields;
}
